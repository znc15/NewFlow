...
- GitHub Release: `https://github.com/znc15/NewFlow/releases`
- 直接下载命令：
...
这样可以确保协议文件、客户端配置和本地运行状态规则与当前版本保持一致。

### 更新检查机制

- 更新结果缓存 24 小时
- 缓存位置：`.flowpilot/update-cache.json`
- 首次运行或缓存过期时会请求 GitHub API
- 强制重新检查：删除 `.flowpilot/update-cache.json` 后再执行任意命令

### 哪些命令不会触发更新检查

- `node flow.js version`
- `node flow.js -h`
- `node flow.js --help`
...
**Q: 更新后需要重新初始化吗？**  
A: 需要，执行 `node flow.js init`。

**Q: 没有网络怎么办？**  
A: 会自动跳过更新检查，不影响命令执行。

**Q: 如何手动检查新版本？**  
A: 删除 `.flowpilot/update-cache.json` 后执行任意命令，或者直接访问 release 页面。
